/**
 * Write PortalAuthDialog.vue. Run: node scripts/write-portal-auth-dialog.js
 */
const fs = require('fs')
const path = require('path')

const file = path.join(__dirname, '../src/layout-portal/components/PortalAuthDialog.vue')

// Fix registerRules messages - use unicode in source file directly via \u in template strings in node
const L = {
  usernameLen: '\u7528\u6237\u8d26\u53f7\u957f\u5ea6\u5fc5\u987b\u4ecb\u4e8e 2 \u548c 20 \u4e4b\u95f4',
  confirmPwd: '\u8bf7\u518d\u6b21\u8f93\u5165\u5bc6\u7801',
...
const content = `<template>
  ...
  >
    ...
    <div class="auth-footer">
      ...
      <router-link to="/login" class="admin-link" @click.native="visible = false">{{ labels.adminLogin }}</router-link>
    </div>
  </el-dialog>
</template>
...
const MSG_USERNAME_LEN = '${L.usernameLen}'
const MSG_CONFIRM_PWD = '${L.confirmPwd}'
const MSG_PWD_MISMATCH = '${L.pwdMismatch}'
...
.auth-tabs { display: flex; gap: 8px; margin-bottom: 20px; padding: 4px; background: #EFF6FF; border-radius: 10px; }
.auth-tab {
  ...
  &.active { color: #fff; background: linear-gradient(135deg, $primary, $primary-light); box-shadow: 0 2px 8px rgba(37, 99, 235, 0.25); }
}
...
console.log('wrote PortalAuthDialog.vue')
