...
const out = path.join(__dirname, '../src/utils/docxQuestionFrame.js')

const content = `const QUESTION_START = /^[\\s\\u3000]*(?:\\(\\d+\\)|\\uff08\\d+\\uff09|\\d+[.\\uFF0E\\u3001)\\uFF09]|\\u7b2c\\d+\\u9898)/
const SECTION_HEADER = /^[\\s\\u3000]*(?:[\\u4e00-\\u9fa5]{1,6}[\\u3001\\.\\uFF0E\\uFF0E]|\\u7b2c[\\u4e00-\\u9fa5\\d]+[\\u90e8\\u5206\\u8282])/
...
function findLastOptionIndex(nodes, from, to) {
  ...
  for (let i = from; i <= to; i++) {
    if (nodeContainsOption(nodes[i])) last = i
  }
  return last
}
