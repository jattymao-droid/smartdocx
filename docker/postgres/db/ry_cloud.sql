...
insert into sys_config values(9,  '门户首页-Banner模式',           'portal.home.banner.mode',          'none',          'Y', 'admin', CURRENT_TIMESTAMP, '', null, 'none=默认渐变, image=背景图, video=背景视频');
insert into sys_config values(10, '门户首页-Banner图片',           'portal.home.banner.imageUrl',      '',              'Y', 'admin', CURRENT_TIMESTAMP, '', null, '图片URL，如 /profile/upload/... 或完整 http 地址');
insert into sys_config values(11, '门户首页-Banner视频',           'portal.home.banner.videoUrl',      '',              'Y', 'admin', CURRENT_TIMESTAMP, '', null, 'mp4/webm 视频地址');
insert into sys_config values(12, '门户首页-Banner视频封面',       'portal.home.banner.videoPoster',   '',              'Y', 'admin', CURRENT_TIMESTAMP, '', null, '视频加载前封面图');
insert into sys_config values(13, '门户首页-Banner遮罩',           'portal.home.banner.overlay',       '0.42',          'Y', 'admin', CURRENT_TIMESTAMP, '', null, '0-1，越大文字对比越强，建议 0.3-0.55');
...
SELECT setval(pg_get_serial_sequence('sys_user', 'user_id'), COALESCE((SELECT MAX(user_id) FROM sys_user), 1));
SELECT setval(pg_get_serial_sequence('sys_post', 'post_id'), COALESCE((SELECT MAX(post_id) FROM sys_post), 1));
SELECT setval(pg_get_serial_sequence('sys_role', 'role_id'), COALESCE((SELECT MAX(role_id) FROM sys_role), 1));
...
SELECT setval(pg_get_serial_sequence('sys_notice', 'notice_id'), COALESCE((SELECT MAX(notice_id) FROM sys_notice), 1));
SELECT setval(pg_get_serial_sequence('sys_job', 'job_id'), COALESCE((SELECT MAX(job_id) FROM sys_job), 1));
SELECT setval(pg_get_serial_sequence('sys_job_log', 'job_log_id'), COALESCE((SELECT MAX(job_log_id) FROM sys_job_log), 1));
SELECT setval(pg_get_serial_sequence('sys_logininfor', 'info_id'), COALESCE((SELECT MAX(info_id) FROM sys_logininfor), 1));
SELECT setval(pg_get_serial_sequence('sys_notice_read', 'read_id'), COALESCE((SELECT MAX(read_id) FROM sys_notice_read), 1));
SELECT setval(pg_get_serial_sequence('gen_table', 'table_id'), COALESCE((SELECT MAX(table_id) FROM gen_table), 1));
SELECT setval(pg_get_serial_sequence('gen_table_column', 'column_id'), COALESCE((SELECT MAX(column_id) FROM gen_table_column), 1));